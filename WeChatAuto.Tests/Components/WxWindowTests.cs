...
namespace WeChatAuto.Tests.Components
{
    ...
    public class WxWindowTests
    {
        ...
        public void Test_CreateOwnerChatGroup_ChatGroup_NotExist()
        {
            ...
            var client = framework.GetWeChatClient(_wxClientName);
            var window = client.WxMainWindow;
            var result = window.CreateOrUpdateOwnerChatGroup("测试11", new string[] { "AI.Net", "秋收" });
        ...
        public async Task Test_UpdateGroupMemo()
        {
            ...
            var client = framework.GetWeChatClient(_wxClientName);
            var window = client.WxMainWindow;
            var result = window.ChangeOwnerChatGroupMemo("测试09", "测试09新的备注6");
        ...
        public async Task Test_UpdateGroupName()
        {
            ...
            var client = framework.GetWeChatClient(_wxClientName);
            var window = client.WxMainWindow;
            var result = window.ChangeOwnerChatGroupName("测试09", "测试09-01");
        ...
        public async Task Test_AddChatGroupMemberToFriends(string groupName)
        {
            ...
            var client = framework.GetWeChatClient(_wxClientName);
            var window = client.WxMainWindow;
            var result = await window.AddChatGroupMemberToFriends(groupName, new string[] { "AI.Net", "秋收" }, 5, "你好呀", "测试标签");
        ...
        #region 获取微信id
        [Fact(DisplayName = "测试获得当前好友的wxid")]
        public async Task TestGetWxId()
        {
            var framework = _globalFixture.clientFactory;
            var client = framework.GetWeChatClient(_wxClientName);
            var window = client.WxMainWindow;
            var wxid = await window.GetWxid("",true,@"c:\my.png");
        ...
        public async Task TestGetWxIdWithWho(string who)
        {
            ...
            var client = framework.GetWeChatClient(_wxClientName);
            var window = client.WxMainWindow;
            var wxid = await window.GetWxid(who,true,@"c:\guo.png");
            _output.WriteLine(wxid.ToString());
            Assert.NotNull(wxid);
        }
