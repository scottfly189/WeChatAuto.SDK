    ...
    public class SubWin : IWeChatWindow, IDisposable
    {
          ...
          SubWinList subWinList, IServiceProvider serviceProvider)
        {
            ...
            _ChatContent = new ChatContent(_SelfWindow, ChatContentType.SubWindow, "/Pane[2]/Pane/Pane[2]/Pane/Pane",
              this, uiMainThreadInvoker, this._MainWxWindow, _serviceProvider);
        ...
        public void AddMessageListener(Action<MessageContext> callBack, Action<Sender> firstMessageAction = null)
        {
            if (_disposed)
            {
                return;
            ...
            _ChatBodyCache.AddListener(callBack, firstMessageAction);
        }
        /// <summary>
        /// 停止消息监听
        /// </summary>
        ...
        /// <returns></returns>
        public async Task<FriendInfo> GetWxId(bool fetchImage, string avatarPath)
        {
                    ...
                    try
                    {
                        ...
                        if (retryButton.Success)
                        {
                            ...
                            if (retryWxId.Success)
                            {
                                ...
                                //昵称/备注
                                var wxLables = this.SelfWindow.FindFirstByXPath("/Pane[1]/Pane/Pane/Pane/Pane/Pane/Pane/Pane/Pane/Pane/Text[2]")
                                    .GetParent().GetParent().FindAllDescendants(cf => cf.ByControlType(ControlType.Text));
            ...
            List<string> list = _uiMainThreadInvoker.Run(automation =>
            {
                ...
                while (true)
                {
                    ...
                    if (moreButton.Success)
                    {
                        ...
                        {
                            KMSimulatorService.LeftClick(_SelfWindow, moreButton.Result);
                        }
                        else
                        {
        ...
        private void _AddChatGroupFriendsCore(OneOf<string, string[]> memberName, int intervalSecond = 3, string helloText = "", string label = "")
        {
            ...
            _uiMainThreadInvoker.Run(automation =>
            {
                foreach (var who in willAddList)
                {
                    ...
                    var paneRoot = Retry.WhileNull(() => _SelfWindow.FindFirstChild(cf => cf.ByControlType(ControlType.Pane).And(cf.ByClassName("SessionChatRoomDetailWnd"))),
                        TimeSpan.FromSeconds(2), TimeSpan.FromMilliseconds(200))?.Result;
                    if (paneRoot != null)
                    {
                        paneRoot.Focus();
                        ...
                        if (edit != null)
                        {
                            _SearchWillAddFriendCore(who, edit);
                            ...
                            if (listItem != null)
                            {
                                ...
                                if (button != null)
                                {
                                    _ClickAndPopupAddFriendMenuCore(listItem, button);
                                    var addButton = Retry.WhileNull(() => _SelfWindow.FindFirstByXPath("/Pane[1]/Pane[1]/Pane[2]/Pane/Pane/Pane/Pane[2]/Pane[2]/Button[@Name='添加到通讯录']")?.AsButton(),
                                        TimeSpan.FromSeconds(2), TimeSpan.FromMilliseconds(200))?.Result;
                                    if (addButton == null)
                                    {
                                        paneRoot.Focus();
                                        ...
                                        _logger.Error($"未找到添加到通讯录按钮，可能好友不是新好友，历史上添加过，所以没有弹出确认窗口，请手工确认是否添加成功。");
                    ...
                    {
                        _logger.Error($"未找到群聊窗口，无法添加好友: {who}");
                    }
                    Thread.Sleep(intervalSecond * 1000);  //停顿间隔时间
                }
            }).GetAwaiter().GetResult();
        ...
        /// 点击列表项并弹出添加好友菜单
        /// </summary>
        private void _ClickAndPopupAddFriendMenuCore(ListBoxItem listItem, Button button)
        {
            button.DrawHighlightExt();
            button.WaitUntilClickable();
