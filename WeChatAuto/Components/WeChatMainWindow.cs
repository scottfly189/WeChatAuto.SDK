...
namespace WeChatAuto.Components
{
    ...
    public class WeChatMainWindow : IWeChatWindow, IDisposable
    {
        ...
        /// <returns>个人信息<see cref="FriendInfo"/></returns>
        public async Task<FriendInfo> GetWxid(string who, string avatarPath = default)
        {
            FriendInfo info = new FriendInfo();
            if (string.IsNullOrWhiteSpace(who))
            {
                //获取当前聊天窗口的好友的wxid
                info = await __GetCurrentChatWxId(avatarPath).ConfigureAwait(false);
            }
            else
            {
        ...
        private async Task<FriendInfo> __GetCurrentChatWxId(string avatarPath = default)
        {
            try
            {
        ...
        /// <param name="avatarPath">头像保存路径，可以为空，如果为空，就不会保存进指定的目录，但会返回Image,具体查看<seealso cref="FriendInfo"/></param>
        /// <returns>个人信息<see cref="FriendInfo"/></returns>
        public async Task<FriendInfo> GetWxidFromPhoneNumber(string phone, string avatarPath = default)
        {
            FriendInfo info = new FriendInfo();
            try
            {
        ...
        private ChatResponse _CreateChatGroupCore(string groupName, ChatResponse result, List<string> list)
        {
            ...
            _logger.Info("创建临时群名称：" + tempName + "成功!");
            return result;
        }
        ...
        public async Task<ChatResponse> DeleteOwnerChatGroup(string groupName)
        {
            ...
            try
            {
                ...
                await Task.Delay(500);
                var subWin = _SubWinList.GetSubWin(groupName);
                _SubWinList.StopMessageListener(groupName);
                _logger.Info($"停止消息监听: {groupName}");
                result = subWin.DeleteOwnerChatGroup();
                _logger.Info($"删除自有群成功: {groupName}");
                return result;
            }
            catch (Exception ex)
            {
